...
class HexapodCPGEnv(gym.Env):
    ...
    def step(self, action):
        ...
        self.Z = solve_ivp(hopf_oscillator, [(self.current_step)*self.dt, (self.current_step+1)*self.dt], self.Z,
                           args=(self.alpha, self.mu, self.omega, self.k), method='RK45').y[:, -1]
        # self.Z = solve_ivp(hopf_oscillator, [(self.current_step)*self.dt, (self.current_step+1)*self.dt], self.Z,
        #                    args=(self.alpha, self.mu, self.omega, self.k), method='RK45').y[:, -1]
        ...
        # log_dir = "./assets/xyz_logs"        
        # os.makedirs(log_dir, exist_ok=True)
        # log_path = os.path.join(log_dir, "cpg.txt")
    ...
    def joint_mapping(self, A):
        ...
        for idx in range(6):
            ...
            # hip
            hip_target = 0.66*self.Z[2*group_idx]
            ...
            # knee
            knee_target = 0.2 * max(self.Z[2 * group_idx+1],0)
    ...
    def _check_truncated(self):
        ...
        if self.current_step >= 9000:
            print("Truncated due to step limit")  
            return True
        return False
...
def eval_hexapod(env, num_episodes=5):
    """
    评估Hexapod机器人在多个回合中的表现，记录并返回每个回合的平均奖励。
    
    Parameters:
    - env: `HexapodCPGEnv` 环境实例
    - num_episodes: 评估的回合数（默认5回合）
    
    Returns:
    - avg_reward_all_episodes: 所有回合的平均奖励列表
    - overall_avg_reward: 所有回合的整体平均奖励
    """
    avg_reward_all_episodes = []  # 用于存储每个回合的平均奖励
    ...
    for episode in range(num_episodes):
        ...
        while True:
            # 计算并应用CPG动作
            action = np.random.uniform(env.action_space.low, env.action_space.high)  # 随机选择一个动作
            observation, reward, terminated, truncated, info = env.step(action)
            
            # 累加奖励
            total_reward += reward
            # time.sleep(2)  # 可选：添加延迟以便观察
            # 如果达到了终止条件或步数被截断，结束评估
            if terminated or truncated:
                break
        
        # 计算每个回合的平均奖励
        avg_reward = total_reward / (env.current_step) if env.current_step > 0 else 0
        avg_reward_all_episodes.append(avg_reward)
        
        # 输出回合结果
        print(f"Episode {episode + 1}/{num_episodes} complete: Avg Reward = {avg_reward}")
    
    # 计算所有回合的平均奖励
    overall_avg_reward = np.mean(avg_reward_all_episodes)
    
    # 输出评估结果
    print(f"Evaluation complete: \n- Overall Avg Reward: {overall_avg_reward}")
    
    return avg_reward_all_episodes, overall_avg_reward

if __name__ == "__main__":
    # 运行评估
    env = HexapodCPGEnv(render_mode='human')  # 初始化环境
    avg_reward_all_episodes, overall_avg_reward = eval_hexapod(env, num_episodes=1)